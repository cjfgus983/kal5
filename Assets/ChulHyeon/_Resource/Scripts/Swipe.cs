...
public class Swipe : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < pos.Length; i++)
        {
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2)) // 현재 인덱스에 따라
            {
                ...
                for (int j = 0; j < pos.Length; j++)
                {
                    if (j != i)
                    {
                        ...
                        transform.GetChild(j).localScale = Vector2.Lerp(transform.GetChild(j).localScale, new Vector2(0.0f, 0.0f), 0.1f);
                    }
                }
            }
        }
